...
static inline void *internal_array_get(array *const arr, const size_t index) {
  return (byte *)arr->data + (index * arr->elem_size);
}

array array_new_(const void *const data, const size_t elem_size,
                 const size_t length) {
  array arr = array_init(elem_size, length);
  memcpy(arr.data, data, elem_size * length);
  return arr;
}

void *array_set_(array *const arr, const void *const elem, const size_t index) {
...
void array_for_each_(array *const arr, void (*const operation)(void *elem)) {
  const size_t LEN = arr->length;
  for (size_t i = 0; i < LEN; i++) operation(array_get_(arr, i));
}
...
void *array_insert_(array *const arr, const void *const elem, size_t index) {
  if (index >= arr->length) return NULL;
...
void array_clear_(array *const arr) {
