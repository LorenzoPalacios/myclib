...
void test_stack_copy(void) {
  ...
  assert(stack_height(stk1) == TEST_ARR_LEN);
...
void test_stack_peek(void) {
  ...
  assert(*stack_peek(stk1) == TEST_ARR[TEST_ARR_LEN - 1]);
...
void test_stack_pop(void) {
  ...
  assert(stk1_value == TEST_ARR[TEST_ARR_LEN - 1]);
...
void test_stack_push(void) {
  ...
  while (!stack_is_empty(stk)) {
    ...
    for (; i < TEST_ARR_LEN; i++) {
      const int stk_value = *stack_pop(stk);
      assert(stk_value == TEST_ARR[TEST_ARR_LEN - i - 1]);
    }
  }
